...
export const reduxPatternsCheckerTool = {
  ...
  description: 'Analyzes Redux code for best practices, anti-patterns, and performance issues with pagination and filtering.',
  inputSchema: {
    type: 'object',
    properties: {
      code: {
...
export async function handleReduxPatternsCheck(args) {
  ...
  try {
    ...
    const allIssues = await withTimeout(analysisPromise, LINT_TIMEOUT_MS, "Redux Patterns Analysis");
...
async function analyzeReduxPatterns(code, codeType, strict) {
  ...
  const isReduxToolkitCode = code.includes('createSlice') || code.includes('createAsyncThunk') || 
  ...
  try {
    ...
    traverse(ast, {
      ...
      // Check for large switch statements in reducers
      SwitchStatement(path) {
        if (path.node.cases.length > 10) {
          issues.push({
            ...
            suggestion: 'Consider using Redux Toolkit createSlice or splitting into smaller reducers'
          });
        }
      },
      
      // Check for missing default case in reducers
      SwitchStatement(path) {
      ...
      // Check for complex nested state updates
      ...
      ObjectExpression(path) {
        // Skip all Redux Toolkit files as they handle serialization properly
        if (isReduxToolkitCode) {
          return;
        }
        
        if (hasNonSerializableValues(path.node)) {
          issues.push({
...
function isSideEffect(node) {
  if (node.type === 'MemberExpression') {
    const sideEffects = ['console', 'fetch', 'localStorage', 'sessionStorage', 'document', 'window'];
...
function isInReducer(path) {
  ...
  while (current) {
    if (current.block.type === 'FunctionDeclaration' || current.block.type === 'FunctionExpression') {
...
function isInCreateSliceReducers(functionPath) {
  ...
  if (parent?.node.type === 'ObjectProperty') {
    ...
    if (obj?.node.type === 'ObjectExpression') {
      ...
      if (reducersProp?.node.type === 'ObjectProperty' && reducersProp.node.key?.name === 'reducers') {
        ...
        if (createSliceObj?.node.type === 'ObjectExpression') {
          ...
          if (createSliceCall?.node.type === 'CallExpression' && 
...
function isInCreateSliceExtraReducers(functionPath) {
  ...
  while (currentPath) {
    ...
    if (currentPath.node.type === 'CallExpression' && 
      ...
      while (searchPath) {
        if (searchPath.node.type === 'ObjectProperty' && 
...
function isInitialStateInSlice(path) {
  ...
  while (currentPath) {
    ...
    if (currentPath.parent?.type === 'Property' && 
