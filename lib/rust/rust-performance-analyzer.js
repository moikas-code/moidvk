...
export const rustPerformanceAnalyzerTool = {
  ...
  description: 'Analyzes Rust code for performance issues, inefficient patterns, and optimization opportunities. Identifies allocations, cloning, and algorithmic improvements.',
  inputSchema: {
    type: 'object',
    properties: {
      code: {
...
const performanceRules = [
  ...
  },
  {
    ...
    check: (code) => {
      ...
      const blockingInAsync = /async[^{]*\{[^}]*\b(std::thread::sleep|\.read_to_string|\.write_all)\(/g;
...
export async function handleRustPerformanceAnalyzer(args) {
  ...
  try {
    ...
    if (focus !== 'general') {
      ...
      focusedResults = results.filter(r => 
