...
import { SHARING_LEVELS } from './constants.js';
  ...
  sanitize(code) {
    ...
    for (const pattern of allPatterns) {
      ...
      for (const match of matches) {
        ...
        else if (patternStr.includes('password') || patternStr.includes('pass'))
        ...
        else if (patternStr.includes('private') && patternStr.includes('key'))
          detectedTypes.add('Private Key');
        else if (patternStr.includes('database') || patternStr.includes('connection'))
          detectedTypes.add('Database Credential');
        else detectedTypes.add('Sensitive Data');

        // Replace the sensitive data
        sanitized = sanitized.replace(
          match[0],
          match[0].replace(match[1] || match[0], this.redactText),
        );
      }
    }
...
export class SnippetManager {
  ...
  async extractSnippet(params) {
    ...
    if (!boundary) {
      throw new Error(
        `Invalid sharing level: ${sharingLevel}. Use: ${Object.keys(this.boundaryTypes).join(', ')}`,
      );
    }
    ...
    if (lineCount > boundary.maxLines) {
      throw new Error(
        `Snippet exceeds ${sharingLevel} limit of ${boundary.maxLines} lines (has ${lineCount} lines)`,
      );
    }
